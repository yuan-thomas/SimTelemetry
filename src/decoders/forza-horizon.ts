...
class ForzaDecoder {
  ...
  private readonly EXPECTED_SIZE = 324; // Size of the expected UDP packet
  ...
  decode(buffer: ArrayBuffer): ForzaTelemetryFrame | null {
    ...
    try {
      ...
      // V2 additional Fields - Forza Horizon 4/5 only
      const CarCategory = view.getInt32(offset, true); offset += 4;
      const Unknown1 = view.getInt32(offset, true); offset += 4;
      const Unknown2 = view.getInt32(offset, true); offset += 4;
  ...
  getPacketTypeName(packetType: number): string {
    return packetType === 1 ? 'Forza Horizon' : `Unknown_${packetType}`;
  }

  getSupportedPacketTypes(): { type: number; name: string }[] {
    return [{ type: 1, name: 'Forza Horizon' }];
  }
